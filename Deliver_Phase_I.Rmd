...
20.
```{r} #applicationbase~scholarship 
applar<-dplyr::select(ApplicationBase,app_number,Scholarship_app)
regression<-lm(ApplicationBase$app_number~ApplicationBase$Scholarship_app)
summary(regression)
...
```
```{r} #applicationbase ~age 
appage<-dplyr::select(ApplicationBase,app_number,age_at_app)
regression<-lm(ApplicationBase$age_at_app~ApplicationBase$app_number)
summary(regression)
...
21.
```{r}#applicationbase~ gre and gpa
appbase<-dplyr::select(ApplicationBase,identity,app_number,age_at_app,Scholarship_app)
...
22.
```{r} ##applicationbase :app_number,age_at_app,'Credits Req',Scholarship_app (four part)
appbase<-dplyr::select(ApplicationBase,identity,app_number,age_at_app,'Credits Req',Scholarship_app)
regression<-lm(ApplicationBase$app_number~ApplicationBase$age_at_app+ApplicationBase$`Credits Req`+ApplicationBase$Scholarship_app)
summary(regression)
...
hist(ApplicationBase$age_at_app)
hist(ApplicationBase$`Credits Req`)
table(ApplicationBase$permanent_country)
```
